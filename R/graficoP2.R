...
# leer datos
datos_p2 <- read_csv("datosp2.csv")

## regresion lineal
fit = lm(y~x*log(x), data=datos_p2)
...
p + geom_point(size=2) + 
  ...
  annotate("text",x=10000/2,y = 1000000*5*2, hjust=0,label = paste('Valor p = ',formatC(p_value, format = "e", digits = 2))) +
  theme_bw() +
